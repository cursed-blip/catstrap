    ...
    public class Bootstrapper
    {
        ...
        private async Task GetLatestVersionInfo()
        {
            ...
            if (!App.LaunchSettings.VersionFlag.Active
                && !IsStudioLaunch
                && App.Settings.Prop.PinnedVersion is Models.Entities.PinnedClientVersion pin
                && !String.IsNullOrEmpty(pin.VersionGuid))
            {
                App.Logger.WriteLine(LOG_IDENT, $"Following the pinned client version {pin.VersionGuid}");

                App.LaunchSettings.VersionFlag.Active = true;
                App.LaunchSettings.VersionFlag.Data = pin.VersionGuid;
            }
        ...
        private async Task StartRoblox()
        {
            ...
            if (_launchMode == LaunchMode.Player && App.Settings.Prop.EnableAssetProxy)
            {
                AssetProxyManager.PrepareRobloxTrust();

                AssetProxyCache.RefreshIfNeeded();

                int proxyPort = AssetProxyManager.EnsureHost();

                if (proxyPort > 0)
                    App.Logger.WriteLine(LOG_IDENT, $"Asset proxy host is up on {proxyPort}, redirecting {string.Join(", ", AssetProxyManager.InterceptHosts)}");
                else
                    App.Logger.WriteLine(LOG_IDENT, "Asset proxy is enabled but no host started, launching without interception");
            }
            else if (HostsFile.AreEntriesPresent() && AssetProxyManager.GetHostStatus(AssetProxyManager.DefaultPort, 300) is null)
            {
                App.Logger.WriteLine(LOG_IDENT, "Leftover asset proxy hosts entries found; turn the proxy back on or repair them from the Asset Proxy page");
            }
            ...
            bool needWatcher = App.Settings.Prop.EnableActivityTracking || App.Settings.Prop.EnableWindowManipulation || App.LaunchSettings.TestModeFlag.Active || autoclosePids.Any();
        ...
        private async Task<bool> CheckForUpdates()
        {
            ...
            if (App.State.Prop.LastUpdateCheckTime != DateTime.MinValue && DateTime.UtcNow - App.State.Prop.LastUpdateCheckTime < TimeSpan.FromHours(24))
            {
                App.Logger.WriteLine(LOG_IDENT, "Update check was already performed within the last 24 hours, skipping");
                return false;
            }
            ...
            App.State.Prop.LastUpdateCheckTime = DateTime.UtcNow;
            App.State.Save();
