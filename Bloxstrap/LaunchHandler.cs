...
namespace Bloxstrap
{
    public static class LaunchHandler
    {
        ...
        public static void ProcessLaunchArgs()
        {
            ...
            else if (App.LaunchSettings.AssetProxyFlag.Active)
            {
                App.Logger.WriteLine(LOG_IDENT, "Hosting the asset proxy");
                LaunchAssetProxyHost(App.LaunchSettings.AssetProxyFlag.Data);
            }
            else if (App.LaunchSettings.AssetProxyRepairFlag.Active)
            {
                App.Logger.WriteLine(LOG_IDENT, "Repairing the asset proxy hosts entries");
                LaunchAssetProxyRepair();
            }
        ...
        public static void LaunchAssetProxyHost(string? portData)
        {
            const string LOG_IDENT = "LaunchHandler::LaunchAssetProxyHost";

            int port = Integrations.AssetProxy.AssetProxyManager.DefaultPort;

            string data = portData ?? string.Empty;

            bool manageHosts = !data.Contains("diagnostic", StringComparison.OrdinalIgnoreCase);
            string portToken = new(data.TakeWhile(char.IsDigit).ToArray());

            if (int.TryParse(portToken, out int parsedPort) && parsedPort is > 0 and < 65536)
                port = parsedPort;

            App.Logger.WriteLine(LOG_IDENT, $"Asset proxy host starting on port {port} (managing the hosts file: {manageHosts})");

            var host = Task.Run(() => Integrations.AssetProxy.AssetProxyHost.Run(port, manageHosts));

            host.ContinueWith(t =>
            {
                if (t.IsFaulted)
                    App.Logger.WriteException(LOG_IDENT, t.Exception!.GetBaseException());

                App.Terminate();
            });
        }

        public static void LaunchAssetProxyRepair()
        {
            const string LOG_IDENT = "LaunchHandler::LaunchAssetProxyRepair";

            Task.Run(() => Integrations.AssetProxy.AssetProxyManager.RepairHostsEntries()).ContinueWith(t =>
            {
                if (t.IsFaulted)
                    App.Logger.WriteException(LOG_IDENT, t.Exception!.GetBaseException());
