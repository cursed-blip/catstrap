...
namespace Bloxstrap
{
    ...
    {
        internal const string AuthCookieName = ".ROBLOSECURITY";

        internal static readonly Regex AuthCookiePattern = new($@"(\t{AuthCookieName}\t)(.+?)(;|$)", RegexOptions.Compiled);
        ...
        public async Task<HttpResponseMessage> AuthPostWithToken(Uri uri, Func<HttpContent> contentFactory)
        {
            const string LOG_IDENT = "CookiesManager::AuthPostWithToken";

            HttpResponseMessage response = await AuthPost(uri, contentFactory());

            if (response.StatusCode != System.Net.HttpStatusCode.Forbidden)
                return response;

            string? token = response.Headers.TryGetValues("x-csrf-token", out IEnumerable<string>? values)
            ...
            {
                App.Logger.WriteLine(LOG_IDENT, "Roblox asked for a CSRF token but didn't supply one");

                return await AuthPost(uri, contentFactory());
            }
            ...
            retry.Headers.Add("x-csrf-token", token);

            return await AuthRequest(retry);
        }

        public string CookieFilePath => CookiesPath;

        public string? ExportAuthCookie() => String.IsNullOrEmpty(AuthCookie) ? null : AuthCookie;

        public void ForgetLoadedCookie()
        {
