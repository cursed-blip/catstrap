        ...
        public virtual void Load(bool alertFailure = true)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (alertFailure)
                {
                    ...
                    try
                    {
                        string backup = $"{FileLocation}.{DateTime.Now:yyyyMMdd-HHmmss}.bak";

                        File.Copy(FileLocation, backup, true);

                        App.Logger.WriteLine(LOG_IDENT, $"Kept the unreadable file as {backup}");
                    }
                    catch (Exception copyEx)
                    {
                        App.Logger.WriteLine(LOG_IDENT, $"Failed to create a backup of {FileLocation}");
